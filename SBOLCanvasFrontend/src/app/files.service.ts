...
export class FilesService {
  ...
  login(email: string, password:string, server:string): Observable<string>{
    ...
    return this.http.get(this.loginURL, {responseType: 'text', params:params});
  }
  ...
  listCollections(user:string, server:string): Observable<any>{
    ...
    params = params.append("server", server);
    return this.http.get(this.listMyCollectionsURL, {params: params});
  }
  ...
  uploadSBOL(mxGraphXML: string, server: string, collection: string, user: string, filename: string){
    ...
    return this.http.post(this.addToCollectionURL, mxGraphXML, {responseType: 'text', params: params});
  }
